...
The csv file is read as pandas dataframe. State and country name is added to the data frame using aggregate operation. A new location column is added joining the ward name, state, and country name. A function is defined to create two new columns, latitude and longitude; the columns are added after geocoding the location using GeoPy and Google Maps as Nominatim user agents.
